...
const MatchList = ({ image, results = [], onSelectImage, onReset }) => {
  const [page, setPage] = useState(1);
  const navigation = useNavigation();

  // detect "no results"
  const noResults = !results.length || results.every((r) => isNoResultItem(r));
        ...
        {noResults ? (
          <View style={styles.emptyCard}>
            ...
            <TouchableOpacity
              ...
              onPress={() => (onReset ? onReset() : null)}
              activeOpacity={0.9}
            >
              <Text style={styles.tryAgainText}>Upload Another Photo</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <>
