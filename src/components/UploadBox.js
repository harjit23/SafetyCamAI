...
const toPickedAsset = (img) => ({
  // keep file:// for iOS, add it if missing
  uri: img.path?.startsWith('file://') ? img.path : `file://${img.path}`,
  // prefer picker filename, fallback to last path segment
  fileName: img.filename || (img.path?.split('/').pop() ?? 'photo.jpg'),
  // be explicit for iOS HEIC → JPEG after cropping
  ...
  return (
    <View style={styles.uploadBox}>
      ...
      <View style={styles.noteBox}>
        ...
        <Text style={styles.notee}>For best results, please upload a photo of the person's face :</Text>
        ...
        <Text style={styles.notee2}>The clearer the face, the higher your chances of getting an accurate match.</Text>
      </View>
